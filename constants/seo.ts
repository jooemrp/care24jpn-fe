/**
 * Care 24 Japan — SEO seed metadata and generated-asset manifest.
 *
 * RULES (per project convention, same as every other seed-only `constants/*.ts`):
 * - This file is seed/tooling input only. It is deliberately NOT imported by
 *   runtime route metadata; live titles, descriptions and og:image values are
 *   required from Atlas by `features/seo/pageMetadata.ts`.
 ...
 * brand name. Runtime metadata reads the published legal document heading from
 * Atlas; the literal route entries below remain only for seed/tooling history.
 */
...
/**
 * Site-wide generated-card paths for seed tooling only. Runtime metadata does
 * not read this value: `pageMetadata()` requires the published Atlas
 * `seo.og_image` field and fails closed when it is missing or malformed.
 */
