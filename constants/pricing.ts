/**
 * Care 24 Japan — Seed/type shapes for pricing data.
 *
 * RULES:
 * - Never hardcode ¥ values inside components. Runtime components receive
 *   numeric amounts from Atlas and format them via `features/rates/format-yen`.
 * - All numbers below are the live, client-reviewed prices. Edit here —
 *   never inline in components.
 * - Store seed amounts as plain integers (yen) for Atlas seeding only.
 */
