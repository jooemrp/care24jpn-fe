...
| 4 | 6 | Sticky Phone + Contact CTA after FV | Current branch has a full-width blue/pink desktop rail with a centered `max-w-6xl` CTA grid aligned to the navbar, pink `お問合せ` side, stronger button affordance, and mobile split CTA. | [Production max-width desktop screenshot](../audit-prod-1280-cta.png), [earlier desktop screenshot](../qa-after-cta-maxwidth.png), [mobile screenshot](../qa-final-menu-mobile.png) |
...
| 13 | 15 | Consultation copy must not leave a lone `の` on mobile | Updated Atlas `home-apply.consult_body_mobile` with the client-approved break after `ご家族の`; the production rendering now keeps `「困った」` together on the second line at 320px and 375px. Desktop remains unchanged. | [Production 375px screenshot](../audit-prod-375-apply.png), [production 320px screenshot](../audit-prod-320-apply.png), [previous Atlas screenshot](../audit-after-apply-atlas-375.png), [desktop screenshot](../audit-after-apply-desktop.png) |
| 14 | 16 | ISMS line breaks | Workbook status is OK. | Workbook row 16 |
| 15 | 17 | Bank-transfer payment copy | Updated Atlas `home-pricing-summary.payment_body_mobile` with the client-approved two-line break: `お支払いは` / `銀行振込（前払い）となります。`; the production rendering no longer leaves a lone `す。` at 320px and 375px. | [Production 375px screenshot](../audit-prod-375-payment.png), [production 320px screenshot](../audit-prod-320-payment.png), [previous Atlas screenshot](../audit-after-payment-atlas-375.png), [desktop screenshot](../audit-after-payment-desktop.png) |
| 16 | 18 | Nursing fee/transport notes | Workbook status is OK. | Workbook row 18 |
| 17 | 19 | Pricing link mobile break | Pricing links now break after `詳しくはこちら（料金ページ）を`; desktop stays one line. | [Pricing-link mobile screenshot](../audit-after-pricing-mobile.png) |
...
- Production build (`node node_modules/next/dist/bin/next build --webpack`): **exit 0** (fresh run; TypeScript and 32/32 static pages completed).
- Fresh production server (`node node_modules/next/dist/bin/next start`) returned **HTTP 200** on both `http://127.0.0.1:3000/` and `http://192.168.1.3:3000/`; the rendered HTML contained neither `Application error` nor `Minified React error`.
...
- The production Next server is running on port 3000 for manual phone QA; localhost and LAN requests both returned HTTP 200 with no Application/Minified React error markers.
